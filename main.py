...
FAVORITE_ICON_PATH = Path(__file__).resolve().parent / "assets" / "favorite.svg"
FAVORITE_FILLED_ICON_PATH = Path(__file__).resolve().parent / "assets" / "favorite-filled.svg"
FOLDER_ICON_PATH = Path(__file__).resolve().parent / "assets" / "folder.svg"
...
class MainWindow(QMainWindow):
    ...
    def _build(self) -> None:
        self.setWindowTitle("Vidpick"); self.resize(980, 760); self.setMinimumSize(860, 800); central = QWidget(); central.setObjectName("central_widget"); self.setCentralWidget(central); root = QVBoxLayout(central); root.setContentsMargins(30, 26, 30, 26); root.setSpacing(14)
        ...
        self.output_button = QPushButton(); self.output_button.setObjectName("utility"); self.output_button.setIcon(QIcon(str(FOLDER_ICON_PATH))); self.output_button.setIconSize(QSize(20, 20)); self.output_button.setToolTip("打开 output 文件夹"); self.output_button.setAccessibleName("打开 output 文件夹"); self.output_button.clicked.connect(self._open_output_folder); header.addWidget(self.output_button); header.addWidget(self.collection_status); self.settings_button = QPushButton(); self.settings_button.setObjectName("utility"); self.settings_button.setIcon(QIcon(str(SETTINGS_ICON_PATH))); self.settings_button.setIconSize(QSize(20, 20)); self.settings_button.setToolTip("采集内容设置"); self.settings_button.clicked.connect(self._edit_settings); header.addWidget(self.settings_button)
        ...
        header.addWidget(mode_container); self.favorites_entry = QPushButton(); self.favorites_entry.setObjectName("favorites_entry"); self.favorites_entry.setFixedSize(38, 38); self.favorites_entry.setIcon(QIcon(str(FAVORITE_ICON_PATH))); self.favorites_entry.setIconSize(QSize(20, 20)); self.favorites_entry.setToolTip("收藏博主"); self.favorites_entry.setAccessibleName("收藏博主"); self.favorites_entry.clicked.connect(self._show_favorites); header.addWidget(self.favorites_entry); root.addLayout(header)
    ...
    def _set_favorite_button(self, checked: bool) -> None:
        self.favorite_button.setIcon(QIcon(str(FAVORITE_FILLED_ICON_PATH if checked else FAVORITE_ICON_PATH))); self.favorite_button.setToolTip("取消收藏" if checked else "收藏博主"); self.favorite_button.setProperty("favorite", checked); self.favorite_button.style().unpolish(self.favorite_button); self.favorite_button.style().polish(self.favorite_button)
    ...
    def _open_output_folder(self) -> None:
        ...
        try:
            ...
            self._log(f"打开 output 文件夹失败：{exc}")
            return
