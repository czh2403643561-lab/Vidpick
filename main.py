...
from PySide6.QtWidgets import QApplication, QCheckBox, QDialog, QDialogButtonBox, QGridLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMessageBox, QPlainTextEdit, QProgressBar, QPushButton, QButtonGroup, QScrollArea, QVBoxLayout, QWidget
...
class MainWindow(QMainWindow):
    ...
    def _build(self) -> None:
        ...
        result = QGroupBox("识别结果"); result.setObjectName("result_card"); result_grid = QGridLayout(result); result_grid.setContentsMargins(12, 5, 12, 9); result_grid.setHorizontalSpacing(10); result_grid.setVerticalSpacing(4); self.status = QLabel("未识别"); self.author = QLabel("—"); self.summary = QLabel("—"); self.summary.setWordWrap(True); self.work_type = QLabel("—"); self.detail = QLabel("识别成功后显示作品信息"); self.detail.setWordWrap(True); self.result_field_labels = []
        ...
        result_grid.setColumnStretch(1, 1); info_layout.addWidget(result)
