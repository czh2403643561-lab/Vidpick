...
from PySide6.QtWidgets import QApplication, QCheckBox, QComboBox, QDialog, QDialogButtonBox, QFormLayout, QGridLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMessageBox, QPlainTextEdit, QProgressBar, QPushButton, QScrollArea, QVBoxLayout, QWidget
        ...
        try:
            for index, work in enumerate(self.works, 1):
                    ...
                    try:
                        ...
                        success += 1; self.log.emit(f"成功 {index}/{total}：{content_path.parent.name}/content.txt")
...
class MainWindow(QMainWindow):
    ...
    def _style(self) -> None:
        self.setStyleSheet("QMainWindow,QWidget{background:#f5f6f8;color:#1d1d1f} QGroupBox{background:white;border:1px solid #e1e4e9;border-radius:14px;margin-top:10px;padding-top:12px;font-weight:600} QGroupBox::title{left:14px;padding:0 4px} QLabel#title{font-size:28px;font-weight:700} QLineEdit,QPlainTextEdit,QComboBox{background:#fbfbfc;border:1px solid #dfe2e8;border-radius:10px;padding:9px} QPushButton{background:#edf0f5;border:none;border-radius:10px;padding:10px 16px;font-weight:600} QPushButton#primary{background:#2775e8;color:white} QPushButton:disabled{color:#a8abb2;background:#eceef2} QPushButton#primary:disabled{background:#b8bdc7;color:#737983} QProgressBar{border:none;border-radius:6px;background:#e8ebf1;height:14px;text-align:center} QProgressBar::chunk{background:#2775e8;border-radius:6px}")
    ...
    def _mode_changed(self, index: int) -> None: self.url.setPlaceholderText("粘贴 douyin.com 博主主页分享链接" if index else "粘贴抖音网页版作品地址"); self.select_works.setVisible(bool(index)); self.select_works.setText("选择作品"); self._show_mode_log(); self._reset()
    ...
    def _start(self) -> None:
        ...
        self._log(f"开始批量任务：{len(works)} 项，跳过 {skipped} 项")
    ...
    def _recognized(self, result: object) -> None:
        ...
        if isinstance(result, VideoInfo): self.single = result; self.select_works.setVisible(False); self.author.setText(result.author); self.summary.setText(result.title); self.detail.setText(result.content[:240] + ("…" if len(result.content) > 240 else "")); self.status.setText("成功"); self.step.setText("识别完成，可以开始任务"); self._log(f"识别成功：{result.author} / {result.title}")
            ...
            self.profile = result; self.select_works.setVisible(True); self.author.setText(result.author); self.summary.setText(f"识别到 {len(result.works)} 个公开作品"); self.detail.setText("请选择要采集的作品"); self.status.setText("成功"); self.select_works.setEnabled(True); self._log(f"主页识别成功：{result.author}，{len(result.works)} 项"); self._choose_works()
    ...
    @Slot(object)
    def _saved(self, result: object) -> None: self.output_dir, content_path, when = result; self.progress.setValue(100); self.status.setText("成功"); self.step.setText("任务完成"); self.open_folder.setEnabled(True); self._log(f"已保存：{content_path.parent.name}/content.txt（{when}）")
    @Slot(object)
